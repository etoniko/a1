/**
 * Own antimat for a1 (browser global).
 * Mask: always "***"
 */
(function (global) {
"use strict";
...
function setBadWords(listOrSet) {
  badWordsSet = listOrSet instanceof Set ? listOrSet : new Set(listOrSet || []);
  try { delete badWordsSet._antimatDict; } catch (_) {}
}

function loadFromUrl(url) {
  return fetch(url, { cache: "no-cache" })
    .then((r) => (r.ok ? r.text() : Promise.reject(new Error("word list " + r.status))))
    .then((text) => {
      const set = new Set(
        String(text || "")
          .split("\n")
...
function censor(message) {
  ...
  if (!badWordsSet || badWordsSet.size === 0) return message;
  return censorText(badWordsSet, message);
}

function count(message) {
  if (disabled) return 0;
  return countHits(badWordsSet, message);
}
...
const api = {
  ...
  isReady() { return badWordsSet.size > 0; },
  size() { return badWordsSet.size; }
};
...
try {
  api.loadFromUrl("./word.txt").then((n) => {
    console.info("[antimat] loaded", n, "words");
